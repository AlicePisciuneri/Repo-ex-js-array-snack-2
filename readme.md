...
# SNACK es.3 Stampare ogni titolo in console
...
# SNACK 2   # es.1 
 Il primo libro scontato Creare un array (availableBooks) che contiene tutti i libri disponibili. 
...
## SNACK2 es.2 
— Prezzo scontato (20%) sui libri disponibili
## Obiettivo
...
## SNACK2 es.3 
 Primo libro con prezzo intero (senza centesimi)
### Obiettivo
...
# Snack 3  –es.1
Creazione array authors con map

### Usare map per creare un nuovo array,uso map() perché permette di:scorrere tutti gli elementi di un array creare un nuovo array trasformare ogni elemento Per ogni libro restituisco book.author.
### Risultato ottenuto authors è un array. Ha lo stesso numero di elementi di books. Ogni elemento è un oggetto autore con name e age.Se un autore ha scritto più libri, comparirà più volte nell’array. Verifica tilizzo console.log(authors) per verificare che l’array contenga solo gli autori.

# snack 3  es.2 
# Crea una variabile booleana (areAuthorsAdults) per verificare se gli autori sono tutti maggiorenni.
### Metodo usato Ho usato every() perché serve a verificare se tutti gli elementi di un array rispettano una condizione. every() restituisce sempre un valore booleano:true se tutti passano il controllo false se anche uno solo non lo passa

### Logica del controllo Per ogni libro controllo book.author.age Un a utore è maggiorenne se: age >= 18 
## Creo la variabile booleana areAuthorsAdults usando every() Verifico il risultato in console con: console.log(areAuthorsAdults)


# snack3  es.3 Ordina l’array authors in base all’età, senza creare un nuovo array. (se areAuthorsAdult è true, ordina in ordine crescente, altrimenti in ordine decrescente)
Obiettivo dello snack Devi ordinare l’array authors per età (quindi author.age), senza creare un nuovo array. In più c’è una regola “a semaforo”:
se areAuthorsAdults è true → ordina crescente (dal più giovane al più vecchio) se areAuthorsAdults è false → ordina decrescente (dal più vecchio al più giovane)

Step 1 – Capire cosa significa “senza creare un nuovo array”     Questa frase ti sta dicendo: usa un metodo che modifica l’array originale.
map, filter → creano un array nuovo ❌    il metodo che ordina lo stesso array è quello che “riordina” gli elementi nella stessa lista ✅
Quindi mentalmente: “non voglio una copia, voglio rimettere in ordine le scatole nella stessa fila”.

uso sort()che  non crea un nuovo array: cambia authors direttamente ✅

in questo caso, con l’autore di 17 anni, areAuthorsAdults sarà false, quindi ordinerà in decrescente (50, 35, 20, 17).
...
### Ho usato map() perché serve a: scorrere tutti gli elementi di un array creare un nuovo array trasformare ogni elemento
### Per ogni libro prendo l’età dell’autore con: book.author.age In questo modo ogni libro viene trasformato in un numero (l’età)
### Codice Creo l’array ages usando map() Verifico il risultato in console con: console.log(ages)

# Snack 4- Es.2
###  Ho usato reduce() perché serve a: scorrere tutti gli elementi di un array ottenere un solo valore finale (in questo caso la somma)
### Logica del calcolo Uso un accumulatore che parte da 0
### Per ogni età faccio: accumulatore + age Il valore finale restituito da reduce() è la somma totale
### Codice Creo la variabile agesSum con reduce() Verifico il risultato in console con: console.log(agesSum).
